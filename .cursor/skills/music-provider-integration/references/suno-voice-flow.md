...
- Путать `task_id` и `voice_id` — хранить task_id в `sunoVoiceId` (persona ломается).
- Выставлять `readyForMusicGeneration: true` только по наличию строки в `sunoVoiceId` без `check-voice`.
- Использовать `checkVoiceAvailability` (пробует task_id) вместо `checkVoiceIdAvailability` для persona.
