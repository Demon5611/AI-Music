...
| `ready` | `sunoVoiceId` (voice_id) + успешный `check-voice` → `readyForMusicGeneration: true` |
...
- Помечать `voiceCloneStatus: ready` без успешного `check-voice(voice_id)` — UI покажет ready, generate вернёт 403.
