...
/consent (SunoVoiceVerifyPanel)
  ...
  → POST .../suno-voice/verify
       → POST /api/v1/voice/generate (verifyUrl, style: Male Vocal | Female Vocal)
...
| `ready` | `sunoVoiceId` + успешный **`checkPersonaVoiceAvailability`** → `readyForMusicGeneration: true` |

Подробнее: [docs/suno-voice-persona-source-of-truth.md](../../../docs/suno-voice-persona-source-of-truth.md),
Cursor rule: [.cursor/rules/suno-voice-persona-contract.mdc](../../rules/suno-voice-persona-contract.mdc)
...
- Путать `task_id` и `voice_id` — не хранить **validation** task_id в `sunoVoiceId` до success record-info.
- Выставлять `readyForMusicGeneration: true` только по наличию строки в `sunoVoiceId` без `check-voice`.
- Suno иногда возвращает `record-info.voiceId === taskId` — для persona использовать `checkPersonaVoiceAvailability` (voice_id, затем task_id), а не только `checkVoiceIdAvailability`.
- Отклонять persona только потому что `sunoVoiceId === sunoVoiceTaskId` — при успешном check это валидный persona id.
...
- Передавать `vocalGender` вместе с `personaModel: voice_persona` — Suno может подставить стандартный взрослый голос вместо persona.
- Помечать `voiceCloneStatus: ready` без успешного **`checkPersonaVoiceAvailability`** — UI покажет ready, generate вернёт 403.
- Отправлять в custom mode полный текст песни при `durationSec ~30` — Suno может отклонить задачу; обрезать lyrics по длительности.
- Ожидать callback на `localhost` — polling обязателен в dev.

## Music generate — проверенный контракт (не ломать)

Reference: [docs/suno-voice-persona-source-of-truth.md](../../../docs/suno-voice-persona-source-of-truth.md).

| Поле Suno | Значение |
| --------- | -------- |
| `personaId` | `sunoVoiceId` из `record-info.voiceId` (может совпадать с taskId) |
| `personaModel` | `voice_persona` |
| `model` | `SUNO_VOICE_MODEL` (V5 / V5_5) |
| `vocalGender` | **не отправлять** |
| `style` | без `*vocal*` тегов; duration hints из `applySunoDurationHints` |

Lyrics API — без `personaId`; `User.vocalGender` только для текста.

**Generate Persona** (`/generate/generate-persona`) — не использовать для user voice sample flow.
