...
/music-create
  → POST /api/music/generate с personaId + model V5
```
...
flowchart TD
  ...
  F --> G[/music-create: генерация с persona]
```
...
> Сначала запишите свободный образец голоса. На следующем шаге Suno попросит повторить короткую фразу напевом — это нужно для верификации и создания вашего AI-голоса.

**`/consent`** (уже есть):

> Запишите фразу ниже — лучше напевом, не просто речью
